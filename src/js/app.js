...
var getQuote = function() {
    $.ajax({
        ...
        success: function(data) {
            ...
            setQuote(object.quote, object.author);
        },
        error: function() {
...
var getImages = function(data) {
    ...
    console.log("Post #: " + randNum);

    var imgUrl;

    // doing some error checking
    if (images[randNum].data.preview != undefined) {
        imgUrl = images[randNum].data.preview.images[0].source.url;
    } else {
        imgUrl = "default.jpg";
    }
...
var forecastItem = function(num, item) {
    $('#' + num + ' span').text(item.day);
    $('#' + num + ' i').addClass('active wi-yahoo-'+item.code);
    if (weatherUnits === 'F') {
        $('#' + num + ' p').text(Math.round(item.high));
    } else {
        $('#' + num + ' p').text(Math.round((item.high - 32) * (5/9)));
    }
};
